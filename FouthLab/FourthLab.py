...
def drawTriangleGouraud(img_mat, z_buffer, x0, y0, z0, x1, y1, z1, x2, y2, z2, i0, i1, i2, u0, v0, u1, v1, u2, v2):
    ...
    for y in range(ymin, ymax + 1):
        for x in range(xmin, xmax + 1):
            ...
            if lambda0 >= 0 and lambda1 >= 0 and lambda2 >= 0:
                ...
                if z < z_buffer[y][x]:
                    intensity = -(lambda0 * i0 + lambda1 * i1 + lambda2 * i2) * 225
                    intensity = max(0, min(255, intensity))

                    color = (int(42), int(intensity), int(intensity))
                    img_mat[y][x] = color
                    z_buffer[y][x] = z


def draw_triangle_model(img_mat, z_buffer, fv, vertex_normals, vt):
    v0_idx = fv[int(f_vt[0])] - 1
    v1_idx = fv[int(f_vt[1])] - 1
    v2_idx = fv[2] - 1
    ...
    drawTriangleGouraud(img_mat, z_buffer, sx1, sy1, sz1, sx2, sy2, sz2, sx3, sy3, sz3, i0, i1, i2, u0, v0, u1, v1, u2, v2)
...
for s in file:
    ...
    if spl[0] == "f":
        ...
        fv.append(v_index)
        vt_index = [int(spl.split("/")[1]) for spl in spl[1:]]
        f_vt.append(vt_index)
    if spl[0] == "vt":
...
Width_texture = texture.shape[0]
Height_texture = texture.shape[1]
for pg in fv:
        draw_triangle_model(img_mat, z_buffer, pg, vertex_normals, vt)

img = Image.fromarray(img_mat, mode='RGB')
img = ImageOps.flip(img)
img.save('img_gouraud.png')
img.show()